...
def Sensar():
    ...
    while True:
        # Obtener el valor actual de la progressBar de GUI 4
        inventario = gui4.progressBar.value()

        # Generar valores de temperatura, humedad y amperaje
        temp = round(random.uniform(09.0, 11.0), 2)
        hum = round(random.uniform(45.0, 50.0), 2)
        amp = round(random.uniform(0.0, 2.0), 2)
        datos_sensor = {
            "temperatura": temp,
            "humedad": hum,
            "amperaje": amp,
            "inventario": inventario  # Agregar inventario al payload
        }

        # Actualizar los labels en la GUI 8
        gui8.temp1.setText(f"Temperatura: {temp}°C")
        gui8.hum2.setText(f"Humedad: {hum}%")
        gui8.amp2.setText(f"Amperaje: {amp}A")

        # Publicar los datos en el topic de MQTT
        cliente.publish(topic, json.dumps(datos_sensor))
        print(f"Datos enviados: {datos_sensor}")

        # Conectar a la base de datos y guardar los datos en la tabla
        conn = conectar_base_datos()
        if conn:
            cursor = conn.cursor()
            insert_query = """
            ...
            """
            try:
                cursor.execute(insert_query, (temp, hum, amp))
                conn.commit()  # Guardar los cambios
                print(f"Datos guardados en la base de datos: {datos_sensor}")
